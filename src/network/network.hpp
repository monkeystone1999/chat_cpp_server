...
#include <vector>
template <typename T>
concept SessionManager = requires(T t, int sock, const std::string &ip) {
...
public:
  ...
  template <typename T, typename... Args> int run(T func, Args... args) {
    ...
    while (1) {
      int events_on = epoll_wait(epfd, ev_list.data(), 100, 0);
  ...
  template <SessionManager T> int run(T &t) {
    /// SessionManager 를 만들어서 Session Manager 안에 Session 을 두자
    if (init() < 0)
      return -1;
    std::vector<struct epoll_event> ev_list(epoll_size);
    while (1) {
      ...
      for (int i = 0; i < events_on; ++i) {
        ...
        if (it ==
          ...
          t.onAccept(clientSockFD, client_info);
        } else {
          t.onRead();
        }
      }
